...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/09/08 12:36:30 by gbright           #+#    #+#             */
/*   Updated: 2020/09/08 12:36:31 by gbright          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		print_status(t_philo *ph, char *s)
{
	...
	while (--time >= 0)
		mtime += 1000;
	pthread_mutex_lock(&ph->common->op_mutex);
	...
	while (*s)
		write(1, s++, 1);
	write(1, "\n", 1);
	pthread_mutex_unlock(&ph->common->op_mutex);
}
